"""
Method comparison — faithful backend implementations with true prompt concatenation.
...
def run_method(method_name, n_nodes, n_steps, seed, budget_usd, tau, out_dir):
    ...
    actual_steps = 0
    run_id = f"{method_name}_v2_scale_free_v{n_nodes}_s{seed}"
...
def main():
    ...
    with open(os.path.join(args.out_dir, "summary_v2.json"), "w") as f:
