"""
Memory backends for method comparison.
...
# ==============================================================================
# B8 CTWM — five-feature core/tail memory
# ==============================================================================
...
class B8_CTWM(BaseMemory):
    ...
    def context_string(self, current_state):
        ...
        for t in core:
            e = self.entries[t]
            core_parts.append(f"{e['prev']}->{e['action']}->{e['next']}(c={e.get('c', 0):.2f})")
        tail_parts = []
        for t in tail:
            e = self.entries[t]
            tail_parts.append(f"{e['prev']}->{e['action']}->{e['next']}")
        core_str = "core:" + ";".join(core_parts) if core_parts else "core:(empty)"
        tail_str = "tail:" + ";".join(tail_parts) if tail_parts else "tail:(empty)"
